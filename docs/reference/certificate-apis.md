# Certificate API Reference

This reference links the existing detailed API pages.  The conceptual entry
point is [Proof Templates](../proof-templates/overview.md).

## Template APIs

- [Table Certificates](../proof-templates/table-certificates.md)
- [Asymptotic Envelopes](../proof-templates/asymptotic-envelopes.md)
- [Pointwise Envelopes](../proof-templates/pointwise-envelopes.md)
- [ConstantFactory](../proof-templates/constant-factory.md)
- [QProduct Finite Integrals](../proof-templates/qproduct-finite-integrals.md)
- [Contour Shift](../proof-templates/contour-shift.md)

## Detailed Existing References

- [Certificate Overview](../certificates/overview.md)
- [ANT Certificates](../certificates/ant.md)
- [Asymptotic Envelope Certificates](../certificates/ant-asymp.md)
- [Chebyshev Certificates](../certificates/chebyshev.md)
- [QProduct Certificates](../certificates/qproduct.md)
- [ConstantFactory Certificates](../certificates/constants.md)
- [Contour-Shift Certificates](../certificates/contour-shift.md)
